==Nonprofit API
Nonprofit API is an project to provide developer access to available nonprofit information including electronic donation information. The intent of the project to build applications that benefit nonprofits.
...
Nonprofit API is currently an REST api for access to Publication 78 which returns the results in a JSON format. Currently, working on including email address to use for electronic donations via Dwolla and PayPal

GET search
Returns available nonprofit information that match a specified query.

Resource URL
http://nonprofitapi.org/search.json
...
==Example Request
GET http://nonprofitapi.org/search.json?city=San+Francisco&state=CA Returns all nonprofit organizations registered in San Francisco, CA

==TO DO
* Add supplemental data table (includes website, dwolla email address, paypal email address, link to logo, phone number)
* Dwolla Integration (authenticate and verify email belongs to nonprofit)
